        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mDeleteBtnState == 0) {
                    ...
                    mDetailViewHolder = (DetailViewHolder) getChildViewHolder(view);

                    mItemLayout = mDetailViewHolder.mLayout;
                    final String receiverName = mDetailViewHolder.getMessage().getReceiver();
                    final String courierName = mDetailViewHolder.getMessage().getSender();
                    mPosition = mDetailViewHolder.getAdapterPosition();

                    mDeleteTextView = (TextView) mItemLayout.findViewById(R.id.item_delete);
                    mMaxLength = mDeleteTextView.getWidth();
                    mDeleteTextView.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            mListener.onDeleteClick(mPosition, receiverName, courierName);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(dx) > Math.abs(dy)) {//左边界检测
                    ...
                    mItemLayout.scrollBy(dx, 0);//item随手指滑动
                }
                break;

            case MotionEvent.ACTION_UP:
                if (!isItemMoving && !isDragging && mListener != null) {
                    mListener.onItemClick(mDetailViewHolder, mPosition);
                }
                isItemMoving = false;
